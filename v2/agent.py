...
FILTER_GUIDE_OPERATOR = (
    """
    | Filter operator | Behaviour | Filters JSON example |
    |-----------------|-----------|----------------------|
    | **NOT** | Matches rows where the field value **is not** the given value. | `"filters": [ { "key": "color NOT", "value": "red" } ]` |
    | **<** | Field value **less than** the given value. | `"filters": [ { "key": "month <", "value": 9 } ]` |
    | **<=** | Field value **less than or equal** to the given value. | `"filters": [ { "key": "price <=", "value": 200 } ]` |
    | **>** | Field value **greater than** the given value. | `"filters": [ { "key": "price >", "value": 200 } ]` |
    | **>=** | Field value **greater than or equal** to the given value. | `"filters": [ { "key": "price >=", "value": 200 } ]` |
    | **OR** | Checks if **any** of the sub-columns has a listed value. | `"filters": [ { "key": "color1 OR color2", "value": ["red", "blue"] } ]` |
    | **LIKE** | Token-level match inside strings. | `"filters": [ { "key": "title LIKE", "value": "hello" } ]` |
    | *(no operator)* | Exact match; if the value is a list, acts as “IN (…)”. | `"filters": [ { "key": "id", "value": [10, 30] } ]` |

    ### Example showing implicit **AND** between filter objects
    ```json
    {
    "query": "bridge maintenance",
    "filters": [
        { "key": "category", "value": "Construction" },
        { "key": "year >=",  "value": 2024 }
    ]
    }
    """
...
class SafeVectorSearchRetrieverTool(VectorSearchRetrieverTool):
    ...
    def _run(self, query: str, filters: Optional[list] = None):
        ...
        try:
            ...
            if not docs:
                return {"status": "NO_RESULTS", "num_results": 0, "docs": []}
                

            # --- 2️⃣  Convert documents to lightweight strings ---------
            return {"status": "OK", "num_results": len(docs), "docs": docs}
        
        except Exception as e:
            raise Exception(f"Error: {e}, docs: {docs}")
